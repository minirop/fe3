...
while True:
	...
	while True:
		...
		print (".db {} {} LEVEL_{:02} {}".format(get_unit(rom[i]), get_class(rom[i+1]), rom[i+2], get_name(rom[i+3])))
		print (".db ${:02X} ${:02X} {} {}".format(rom[i+4], rom[i+5], unknown(rom[i+6]), get_portrait(rom[i+7])))
		print (".db {} {} {} {}".format(get_item(rom[i+8]), get_item(rom[i+9]), get_item(rom[i+10]), get_item(rom[i+11])))
		print (".db {} {} {} {}".format(get_item(rom[i+12]), get_item(rom[i+13]), get_item(rom[i+14]), get_item(rom[i+15])))
		print (".db {} {} {}\n".format(get_ai(rom[i+16]), get_ai(rom[i+17]), get_ai(rom[i+18])))

		i += 19
	print (".db UNDEFINED ; no more characters\n")
	i += 1

print (".db UNDEFINED ; no more chapters")
